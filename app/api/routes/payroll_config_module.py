...
router = APIRouter(prefix="/api/web/payroll_config", tags=["payroll-config"])
...
@router.get("/get_payroll_config")
async def get_payroll_config(current_user: dict = Depends(get_current_user)):
...
@router.post("/save_payroll_config")
async def save_payroll_config(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    def action():
        ...
        return format_response(
            success=True,
            msg="Payroll configuration saved successfully",
