...
from app.services.attendance_config_service import (
...
router = APIRouter(prefix="/api/web/attendance_config", tags=["attendance-config"])
...
def handle_service_call(fn):
    try:
        return fn()
    except AttendanceConfigError as exc:
...
@router.get("/attendance_config")
async def get_attendance_config(current_user: dict = Depends(get_current_user)):
...
@router.post("/save_attendance_config")
async def save_attendance_config(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    def action():
        ...
        return format_response(
            success=True,
            msg="Attendance configuration saved successfully",
