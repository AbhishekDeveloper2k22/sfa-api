...
@router.post("/analytics_common_header")
async def analytics_common_header(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if not result.get("success"):
            return format_response(
                success=False,
                msg=result.get("message", "Failed to get analytics overview"),
                statuscode=400,
                data={"error": result.get("error", {})}
            )
        ...
        return format_response(
            success=True,
            msg=result.get("message", "Analytics overview retrieved successfully"),
            statuscode=200,
            data=result.get("data", {})
        )
    except Exception as e:
